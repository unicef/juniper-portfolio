...
import { Dialog, DialogContent, Grid, Button, useMediaQuery, Tooltip } from '@material-ui/core'
...
export default function TransactionProof(props: any) {
    ...
    return (
        ...
        >
            ...
            <DialogContent > 
                <Grid container>
                    <Grid xs={4} className={classes.sender} item>{props.sender}</Grid>
                    <Grid xs={3} className={classes.recipient} item>{props.recipient}</Grid>
                    <Grid xs={3} className={classes.amount} item>{props.amount}</Grid>
                </Grid>{/* sender, rec, amount */}
                <Grid container>
                    <Grid xs={4} className={classes.senderDescription} item>{props.senderDescription}</Grid>
                    <Grid xs={3} className={classes.recipientDescription} item>{props.recipientDescription}</Grid>
                    <Grid xs={3} className={classes.amountDescription} item>{props.amountDescription}</Grid>
                </Grid>{/* sender, rec, amount details */}
                <Grid container>
                    <Grid xs={7} item><img className={classes.progressBar} src='./progressbar_3.svg' alt='invest scale' /></Grid>
                </Grid>{/* diagram */}
                <span style={{paddingBottom:'1.5px'}} className={classes.amountDescription}>TRANSACTION DETAILS</span>
                <hr/>
                <Grid container style={{paddingTop:'25px', paddingBottom:'46px'}}>
                    <Grid container>
                        <Grid item xs={2} className={classes.senderDescription} >FROM</Grid>
                        <Grid item xs={2} className={classes.senderDescription} >RECIPIENT</Grid>
