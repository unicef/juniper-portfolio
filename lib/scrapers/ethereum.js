...
class EthereumWalletScraper {
  ...
  async scrapeTransactionData(
    ...
    this.logger.debug(isUnicef)
    this.logger.debug(multisigOwners)
    this.logger.debug(tracked)
    ...
    if (Array.isArray(internalTxData)) {
  ...
  async getAccountMap() {
    ...
    accounts.forEach((account) => {
      account.addresses.forEach((address) => {
        map[address.address.toLowerCase()] = account.name;
      });
    });
  ...
  async saveTransactionData(address, tx, isUnicef, multisigOwnerLookup = {}) {
    ...
    let rate = await this.db.getNearestPrice(this.symbol, date);
    ...
    const newTx = {
      ...
      currency: "Ethereum",
      symbol: this.symbol,
      source: accountMap[tx.from.toLowerCase()],
      destination: accountMap[tx.to.toLowerCase()],
      to: tx.to,
      from: tx.from,
